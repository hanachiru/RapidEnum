...
public static class RapidEnumTemplate
{
    ...
    {
        string ValueArms(Func<EnumMemberInfo, string> selector) =>
            Join(context.DistinctEnumMembers, SwitchArmIndent, selector);

        string SwitchArms(Func<EnumMemberInfo, string> selector) =>
            Join(context.EnumMembers, SwitchArmIndent, selector);

        string ArrayElements(Func<EnumMemberInfo, string> selector) =>
            Join(context.EnumMembers, ArrayElementIndent, selector);
                  ...
                  {{(!string.IsNullOrEmpty(context.NameSpace) ?
                    ...
                    {{context.Accessibility}} static partial class {{context.ClassName}}
                    {
                        ...
                        public static string ToStringFast(this {{context.EnumFullName}} value)
                        {
                            return value switch
                            {
                                {{ValueArms(x => $"global::{x.FullName} => nameof(global::{x.FullName}),")}}
                                _ => value.ToString()
                            };
                        }
                        ...
                        public static bool IsDefined({{context.EnumFullName}} value)
                        {
                            return value switch
                            {
                                {{ValueArms(x => $"global::{x.FullName} => true,")}}
                                _ => false,
                            };
                        }
                        ...
                        public static string? GetEnumMemberValue(this {{context.EnumFullName}} value)
                        {
                            return value switch
                            {
                                {{ValueArms(x => $"global::{x.FullName} => {ToLiteral(x.EnumMemberValue)},")}}
                                _ => null
                            };
                        }
                    }
    ...
    {
        return string.Join(separator, (members ?? []).Select(selector));
    }
